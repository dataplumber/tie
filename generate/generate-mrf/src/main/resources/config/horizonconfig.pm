...
### =============================================================== ###
#                                                                     #
#  The GIBS MRF Generation Launcher Configuration                     #
#                                                                     #
#  Function:                                                          #
#  This module provides utility subroutines for configurating the     #
#  GIBS MRF Generation Launcher command line programs.                #
#                                                                     #
#  Assumptions:                                                       #
...
#                                                                     #
#  Modifications:                                                     #
#  Sep. 2, 2013 C. Alarcon {Christian.Alarcon@jpl.nasa.gov}           #
#                                                                     #
### =============================================================== ###
#
# $Id: $
...
use constant DEFAULT_JRE_VERSION => 1.7;
...
#
sub getJVMArgs {
   my ($classpath, $debug, $cwd) = @_;
   return &getJVMArgsCheckGUI ($classpath, 0, $debug, $cwd);
}
...
sub getJVMArgsCheckGUI {
   ...
   my ($classpath, $gui, $debug, $cwd) = @_;

   my $horizon = abs_path(File::Spec->catdir($cwd, "../config"));
   $classpath = &getClasspath ($classpath, $debug, $cwd);
...
sub getClasspath {
   ...
   my $horizon = File::Spec->catdir($cwd, "..", "config");
   my $seperator = (($^O eq "MSWin32") || 
                   ($^O eq "dos") || 
                   ($^O eq "cygwin")) ? ";" : ":";
                                                                                           
   my $horizonjarpattern = 
      abs_path(File::Spec->catdir($cwd, "..", "lib", "*.jar"));
   my @horizonjarfiles =glob($horizonjarpattern);
   print "[DEBUG] MRFGEN jarfiles=@horizonjarfiles\n" if ($debug);
