...
/**
 * The gov.nasa.gibs.tie.handlers.amsr2.AMSR2 data handler for ingesting file products.  It also extracts
 * metadata from the ingested product for cataloging
 *
 * @author T. Huang
 * @version $Id: $
 */
public class AMSR2Handler implements Worker {
   private static Log logger = LogFactory.getLog(gov.nasa.gibs.tie.handlers.amsr2.AMSR2Handler.class)

   gov.nasa.gibs.tie.handlers.amsr2.AMSR2Configurator config
   boolean error = false
   boolean batch = false
    ...
    */
   public AMSR2Handler(String[] args) {
      logger.debug("gov.nasa.gibs.tie.handlers.amsr2.AMSR2Handler creating configurator")
      this.config = new gov.nasa.gibs.tie.handlers.amsr2.AMSR2Configurator(args)
      logger.debug("gov.nasa.gibs.tie.handlers.amsr2.AMSR2Handler setup configurator to load product type info.")
   ...
   public static void main(String[] args) {
      ...
      gov.nasa.gibs.tie.handlers.amsr2.AMSR2Handler handler = new gov.nasa.gibs.tie.handlers.amsr2.AMSR2Handler(args)
      if (handler.hasError()) {
         System.exit(-1)
      }
      handler.setup()
      ...
      File shutdown = new File("/tmp/amsr2_shutdown")
      ...
      logger.info("gov.nasa.gibs.tie.handlers.amsr2.AMSR2 Handler shutting down...")
