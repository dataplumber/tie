...
/**
 * Implementation of Metadata Harvester for the gov.nasa.gibs.tie.handlers.amsr2.AMSR2 imagery products
 *
 * @author T. Huang
 * @version $Id: $
 */
class AMSR2Metadata implements MetadataHarvester {
   private static Log logger = LogFactory.getLog(gov.nasa.gibs.tie.handlers.amsr2.AMSR2Metadata.class)

   @Override
   public ServiceProfile createServiceProfile(Product product)
   throws ServiceProfileException {

      if (!(product.productType instanceof gov.nasa.gibs.tie.handlers.amsr2.AMSR2ProductType)) {
         throw new ServiceProfileException("Invalid input product type.")
      }

      gov.nasa.gibs.tie.handlers.amsr2.AMSR2ProductType productType = product.productType as gov.nasa.gibs.tie.handlers.amsr2.AMSR2ProductType
      ...
      origin.name = "gov.nasa.gibs.tie.handlers.amsr2.AMSR2 Data Handler"
      ...
      acquire.agent = 'gov.nasa.gibs.tie.handlers.amsr2.AMSR2_HANDLER'
      acquire.operation = 'ACQUIRE'

      SPOperation ingestOpt = header.createOperation()
      header.addOperation(ingestOpt)
      ingestOpt.agent = 'gov.nasa.gibs.tie.handlers.amsr2.AMSR2_HANDLER'
