...
/**
 * Implementation of the gov.nasa.gibs.tie.handlers.amsr2.AMSR2 Product Type
 *
 * @author T. Huang
 * @version $Id: $
 */
@ToString(includeNames = true, includeFields = true)
class AMSR2ProductType extends ProductTypeImpl implements Worker {

   private static Log _logger = LogFactory.getLog(gov.nasa.gibs.tie.handlers.amsr2.AMSR2ProductType.class)
   ...
   private gov.nasa.gibs.tie.handlers.amsr2.AMSR2DataCrawler mdc

   public AMSR2ProductType(String name) {
      this.name = name
   ...
   void run() {
      // do the work
      gov.nasa.gibs.tie.handlers.amsr2.AMSR2FileHandler handler = new gov.nasa.gibs.tie.handlers.amsr2.AMSR2FileHandler(this)
      if (!mdc) {
         mdc =
            new gov.nasa.gibs.tie.handlers.amsr2.AMSR2DataCrawler(this, [sourceURL].iterator(), this.filesetFilter, handler)
         mdc.run()
      }
      if (!batch) {
         this.endDate = null
