...
/**
 * Implementation of Metadata Harvester for the CMR imagery products
 *
 * @author T. Huang
 * @version $Id: $
 */
class CMRMetadata implements MetadataHarvester {
	private static Log logger = LogFactory.getLog(CMRMetadata.class)

	@Override
	public ServiceProfile createServiceProfile(Product product)
	throws ServiceProfileException {

		if (!(product.productType instanceof CMRProductType)) {
			throw new ServiceProfileException("Invalid input product type.")
		}

		CMRProductType productType = product.productType as CMRProductType
