...
/**
 * Implementation of Metadata Harvester for the MLS imagery products
 *
 * @author T. Huang
 * @version $Id: $
 */
class MLSMetadata implements MetadataHarvester {
   private static Log logger = LogFactory.getLog(MLSMetadata.class)

   @Override
   public ServiceProfile createServiceProfile(Product product)
   throws ServiceProfileException {

      if (!(product.productType instanceof MLSProductType)) {
         throw new ServiceProfileException("Invalid input product type.")
      }

      MLSProductType productType = product.productType as MLSProductType
      ...
      */
      ...
      def matches = fileName =~ /(\d*)d(\d*)/
      String year = matches[1][1]
      String jday = matches[1][2]
