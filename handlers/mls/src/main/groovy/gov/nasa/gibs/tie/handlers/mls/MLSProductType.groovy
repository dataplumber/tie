...
/**
 * Implementation of the MLS Product Type
 *
 * @author T. Huang
 * @version $Id: $
 */
@ToString(includeNames = true, includeFields = true)
class MLSProductType extends ProductTypeImpl implements Worker {

   private static Log _logger = LogFactory.getLog(MLSProductType.class)
   ...
   private MLSDataCrawler mdc

   public MLSProductType(String name) {
      this.name = name
   ...
   void run() {
      // do the work
      MLSFileHandler handler = new MLSFileHandler(this)
      if (!this.mdc) {
         mdc =
            new MLSDataCrawler(this, [sourceURL].iterator(), this.filesetFilter, handler)
         mdc.run()
      }
      if (!batch) {
         this.endDate = null
