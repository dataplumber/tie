...
/**
 * Implementation os MODIS configurator
 *
 * @author T. Huang
 * @version $Id: $
 */
...
public class MODISConfigurator extends ApplicationConfigurator {

   private static Log logger = LogFactory.getLog(MODISConfigurator.class)
   ...
   public MODISConfigurator(String[] args) {
      this.args = args
   ...
   protected boolean _parseArgs(String[] args) {
      ...
      if (options.t) {
         ...
         logger.trace ("userProductType = ${this.userProductType}")
      }
   ...
   protected boolean configure() throws DataHandlerException {
      ...
      this.productTypes.values().each { ProductType pt ->
         ...
         logger.trace ("Product type ${pt.name} ready = ${pt.ready}")
         pt.setup()
      }
