...
/**
 * OMI file handler class
 *
 * @author T. Huang
 * @version $Id: $
 */
class OMIFileHandler implements FileHandler, FileProductHandler {

   private static Log logger = LogFactory.getLog(OMIFileHandler.class)

   private OMIProductType productType

   private SigEvent sigEvent

   public OMIFileHandler(OMIProductType productType) {
   ...
   void process(List<FileProduct> fps) throws DataHandlerException {
      ...
      for (FileProduct filep : fps) {
         def d = (filep.name =~ /(\d{4})m(\d{2})(\d{2})/)
      ...
      for (FileProduct fp : fileProducts) {
         ...
         names << fp.name
         logger.info("Checking file ${productName}:${fp.name} cache status: ${inCache}")
      }
      ...
      if (sh.exists() && sh.listFiles().size() == 0) {
         ...
         if (sp != null) {
            ...
            this.sigEvent.create(EventType.Error, productType.name, 'OMIDataHandler',
                  'OMIDataHandler',
