...
/**
 * Implementation of Metadata Harvester for the OMI imagery products
 *
 * @author T. Huang
 * @version $Id: $
 */
class OMIMetadata implements MetadataHarvester {
   private static Log logger = LogFactory.getLog(OMIMetadata.class)

   @Override
   public ServiceProfile createServiceProfile(Product product)
   throws ServiceProfileException {

      if (!(product.productType instanceof OMIProductType)) {
         throw new ServiceProfileException("Invalid input product type.")
      }

      OMIProductType productType = product.productType as OMIProductType
      ...
      Pattern p = Pattern.compile ("(\\d{4})m(\\d{2})(\\d{2})")
      Matcher m = p.matcher(pngFile.name)
      if (m.find()) {
         Calendar cal = Calendar.instance
         SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd")
         sdf.setTimeZone(TimeZone.getTimeZone('GMT'))
         Date st = sdf.parse("${m.group(1)}-${m.group(2)}-${m.group(3)}}")
         logger.debug("Product start time: ${st}")
         metadata.productStartTime = st.time
         metadata.productStopTime = (st + 1).time - 1  // we assumed this is a daily image
		 metadata.addDataDay(st.getTime())
      } else {
         throw new ServiceProfileException("Bad file naming encountered '${pngFile.name}.  Aborting...")
      }
