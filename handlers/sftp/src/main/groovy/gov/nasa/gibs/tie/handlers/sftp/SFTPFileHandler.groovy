...
/**
 * gov.nasa.gibs.tie.handlers.sftp.SFTP file handler class
 *
 * @author T. Huang
 * @version $Id: $
 */
class SFTPFileHandler implements FileHandler, FileProductHandler {

   private static Log logger = LogFactory.getLog(gov.nasa.gibs.tie.handlers.sftp.SFTPFileHandler.class)

   private gov.nasa.gibs.tie.handlers.sftp.SFTPProductType productType

   private SigEvent sigEvent

   public SFTPFileHandler(gov.nasa.gibs.tie.handlers.sftp.SFTPProductType productType) {
   ...
   void process(List<FileProduct> fps) throws DataHandlerException {
      ...
      for (FileProduct fp : fileProducts) {
         ...
         if (productType.batch || !productType.isInCache(cfi)) {
            inCache = false
            break
         }
         names << fp.name
		 	if (logger.debugEnabled) {
				 if(productType.batch)
				 	logger.debug("${productName} running in batch mode, skipping cache check.")
				 else
	  ...
	  //TODO Find a way to short-circuit this logic iff in batch portion of extended daemon
      if ( !productType.batch && inCache && fileProducts.size() > 0) {
         if (logger.debugEnabled) {
	  ...
	  if (newLocation.exists() && newLocation.listFiles().size() > 0) {
		 ...
		 try{
			...
			this.sigEvent.create(EventType.Error, productType.name, 'gov.nasa.gibs.tie.handlers.sftp.SFTPDataHandler',
				 'gov.nasa.gibs.tie.handlers.sftp.SFTPDataHandler',
         ...
         if (sp != null) {
            ...
            for (FileProduct fp in downloadedTarFileProducts) {
               ...
               if (!productType.batch && productType.updateCache(cfi)) {
