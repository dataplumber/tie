...
/**
 * The gov.nasa.gibs.tie.handlers.sftp.SFTP data handler for ingesting file products.  It also extracts
 * metadata from the ingested product for cataloging
 *
 * @author T. Huang
 * @version $Id: $
 */
public class SFTPHandler implements Worker {
   private static Log logger = LogFactory.getLog(gov.nasa.gibs.tie.handlers.sftp.SFTPHandler.class)

   gov.nasa.gibs.tie.handlers.sftp.SFTPConfigurator config
   boolean error = false
   boolean batch = false
    ...
    */
   public SFTPHandler(String[] args) {
      logger.debug("gov.nasa.gibs.tie.handlers.sftp.SFTPHandler creating configurator")
      this.config = new gov.nasa.gibs.tie.handlers.sftp.SFTPConfigurator(args)
      logger.debug("gov.nasa.gibs.tie.handlers.sftp.SFTPHandler setup configurator to load product type info.")
   ...
   public static void main(String[] args) {
      ...
      gov.nasa.gibs.tie.handlers.sftp.SFTPHandler handler = new gov.nasa.gibs.tie.handlers.sftp.SFTPHandler(args)
      if (handler.hasError()) {
         System.exit(-1)
      }
      handler.setup()
      ...
      File shutdown = new File("/tmp/sftp_shutdown")
      ...
      logger.info("gov.nasa.gibs.tie.handlers.sftp.SFTP Handler shutting down...")
