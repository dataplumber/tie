...
class HorizonConfig(object):
    ...
    #
    # COMMAND LINE Configs/Defaults
    #
    
    # Which env to use
    env = socket.gethostname()[0:socket.gethostname().find(".")]
    # Deploy everything but configs
    noConfig = False
    # Skip all prompts?
    nonInteractive = False
    
    #
    # OTHER Hardcoded configs
    #
    ...
    tieRepoPath = None
    manualTarPath = None
    # Filtered list of sections to process
    filteredSections = []
    # Repo path
    configRepoPath = None
    ...
    def __init__(self, configFile):
        ...
        try:
            ...
            self.configRepoPath = self.configObj.get(HorizonConfig.GLOBAL, "configRepoPath") if self.configObj.get("Global", "configRepoPath") != "" else None
    ...
    def get_sections(self):
        ...
        for section in self.configObj.sections():
            if section != "Default" and section != "Global" and section != "Environments":
                filtered.append(section)
        return filtered
    
    def update_filtered(self):
        try:
            sections_str = self.configObj.get(HorizonConfig.ENV, self.env) if self.configObj.get(HorizonConfig.ENV, self.env) != "" else None
            self.filteredSections = map(str.strip,sections_str.split(","))
        except ConfigParser.NoOptionError as e:
            print_combo(e.message+" - Please manually specify a valid environment with -e", logging.ERROR)
            exit(0)
