...
def setup():
    try:
        print_combo("Creating temporary workspace at "+config.tmpPath)
        os.makedirs(config.tmpPath)
    except OSError:
        print_combo(config.tmpPath + " already exists!")
        pass
    return
    ...
    if config.manualTarPath != None and os.path.isdir(config.manualTarPath):
        ...
        for filePath in filePaths:
            if re.match(".*\.tar\.gz", filePath):
                fullPath = fullPath = os.path.join(config.manualTarPath, filePath)
...
def process_tar(tarPath):
    ...
    hasTopLevel = config.getboolean(section, HorizonConfig.HAS_TOP_LEVEL) if config.getboolean(section, HorizonConfig.HAS_TOP_LEVEL) is not None else True 
    ...
    if hasTopLevel:
        print "Has Top Level: Yes"
    else:
        print "Has Top Level: No or Not Specified"
    print "Base Path:     "+basePath
    print "Extracting To: "+workingDir
    print ""
    #print "*"*30
...
def redistribute(path, section):
    ...
    print ""
    print_combo(str(len(srcDestList))+" files/directories in "+path+" distributed successfully to the following locations:", logging.INFO)
...
def main(argv=None):
    ...
    if args.noconfig:
        print_combo( " ---- NO CONFIGURATION WILL BE INSTALLED")
