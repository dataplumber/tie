...
def clean_up_dir(path, nonInteractive = False ):
    abandonedList = []
...
#     if len(abandonedList) > 0 and nonInteractive is False:
...
#     else:
    try:
        ...
        print_combo("Couldn't delete directory "+path, logging.ERROR)
        return
    print_combo("Auto cleanup of path "+path+" completed successfully", logging.INFO)
...
def copy_file(src, dest, overwriteFlag = True):
    dest_base_path = dest[0:dest.find(os.path.basename(dest))]
    try:
        ...
        print_combo(e.strerror, logging.ERROR)
    except IOError as e:
        print_combo(e.strerror, logging.ERROR)
    except OSError as e:
        print_combo(e.strerror, logging.ERROR)
